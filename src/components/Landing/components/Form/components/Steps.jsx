...
const Steps = ({ isCodeValid }) => {
  ...
  const loginTwitter = async () => {
    try {
      ...
      window.open(
        `https://api.twitter.com/oauth/authorize?oauth_token=${res.token}`
      );
    } catch (e) {
      console.log(e);
    }
  ...
  const loginDiscord = async () => {
    window.open(
      `https://discord.com/oauth2/authorize?client_id=1215997115390496918&response_type=code&redirect_uri=https%3A%2F%2F28c9-78-26-198-199.ngrok-free.app%2Fauth%2Fdiscord%2Fcallback&scope=identify+guilds+guilds.join+email+connections`
    );
  };
